...
public class CommandExpired {
	public CommandExpired(CommandSender p, String[] args) {
		...
		String usage = ReporterGUIM.getString("message.report-expired-use").replace("[command]", "/"+ ValueCustom.getCommandMain() + " expire <id>");
		if (args.length == 2) {
			if (Utili.isNumero(args[1])) {
				try {
					
					MysqlStatus.expired(p,Integer.parseInt(args[1]), true);
