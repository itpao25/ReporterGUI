...
public class CommandDecline {
	...
	public CommandDecline(CommandSender p, String[] args) {
		...
		String usage = ReporterGUIM.getString("message.report-declined-use").replace("[command]", "/"+ ValueCustom.getCommandMain() + " decline <id>");
		if (args.length == 2) {
			if (Utili.isNumero(args[1])) {
				try {
					
					MysqlStatus.decline(p,Integer.parseInt(args[1]));
