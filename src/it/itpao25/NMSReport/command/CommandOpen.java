...
public class CommandOpen {
	public CommandOpen(CommandSender p, String[] args) {
		...
		String usage = ReporterGUIM.getString("message.report-open-use").replace("[command]", "/"+ ValueCustom.getCommandMain() + " open <id>");
		if (args.length == 2) {
			if (Utili.isNumero(args[1])) {
				try {
					
					MysqlStatus.open(p,Integer.parseInt(args[1]));
