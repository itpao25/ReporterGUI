...
public class CommandSolve
{
	// Imposto la segnalazione come completata, quindi imposto
	// Lo stato a 2
	public CommandSolve(CommandSender p, String[] args)
	{
		...
		String usage = ReporterGUIM.getString("message.report-solved-use").replace("[command]", "/"+ ValueCustom.getCommandMain() + " solve <id>");
		
		if (args.length == 2) {
			if (Utili.isNumero(args[1])) {
				try {
					
					MysqlStatus.solve(p,Integer.parseInt(args[1]));
